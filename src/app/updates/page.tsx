...
export default function UpdatesPage() {
  return (
    <div className="flex flex-col min-h-screen bg-slate-950">
      <header className="sticky top-0 z-10 bg-slate-950/80 backdrop-blur-md border-b border-slate-800/50 p-4">
      ...
      <main className="flex-1 max-w-2xl w-full mx-auto px-4 py-12">
        <div className="space-y-12 relative before:absolute before:inset-0 before:start-10 before:h-full before:w-0.5 before:bg-slate-800">
          {updates.map((update, idx) => (
            <div key={idx} className="relative flex items-start gap-6">
              <div className="flex items-center justify-center w-12 h-12 rounded-full border-4 border-slate-950 bg-indigo-500 text-white shrink-0 z-10 shadow-lg shadow-indigo-500/20">
                <History className="w-5 h-5" />
              </div>
              <div className="flex-1 pt-1">
                <div className="p-5 rounded-2xl border border-slate-800 bg-slate-900 shadow-xl">
                  <div className="flex items-center justify-between mb-4">
                    <span className="text-lg font-bold text-indigo-400">{update.version}</span>
                  ...
                  <a
                    ...
                    className="inline-flex items-center gap-2 text-xs font-medium text-slate-400 hover:text-indigo-400 transition-colors bg-slate-950 px-4 py-2 rounded-lg"
                  >
                    عرض الكود (Commit)
                  </a>
                </div>
