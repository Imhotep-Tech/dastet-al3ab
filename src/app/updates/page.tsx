...
import updatesRaw from "@/data/updates-history.json";
...
const updates: Update[] = [...updatesRaw].reverse();
      ...
      <main className="flex-1 max-w-2xl w-full mx-auto px-4 py-12">
        <div className="space-y-12 relative before:absolute before:inset-0 before:start-10 before:h-full before:w-0.5 before:bg-slate-900">
          {updates.map((update, idx) => {
            const features =
              language === "ar" ? update.features_ar : update.features_en;
                ...
                <div className="flex-1 pt-1">
                  <div className="p-5 rounded-2xl border border-slate-900 bg-[#0d0d21] shadow-xl">
                    <div className="flex items-center justify-between mb-4">
                      ...
                      <time className="text-sm font-medium text-slate-500 bg-slate-950 px-3 py-1 rounded-full">
                        {update.date}
                      </time>
                    </div>
                    <ul className="space-y-3 mb-6">
                      {features.map((feature, fIdx) => (
                        <li
                          key={fIdx}
                          className="text-slate-300 text-sm flex items-start gap-3 leading-relaxed"
                        >
